// 1월 (January) Dot flow version - 2월과 동일한 구조, 색상/텍스트만 다름
drawFns['0'] = (p, g, st) => {
  if (!st.inited) {
    st.inited = true;
    st.t = 0;
  ...
  for (let idx = 0; idx < copies; idx++) {
    ...
    const progressRaw = Math.abs(Math.sin(Math.PI * (timeWithDelay / SETTINGS.Duration) * 0.6));
    ...
    for (let i = 0; i < SETTINGS.word.length; i++) {
      g.text(SETTINGS.word[i], xpos, y);
      xpos += g.textWidth(SETTINGS.word[i]) + tracking;
    }
  }
  // flowing dots
  g.fill(SETTINGS.fg);
  ...
  const capY = textAreaBottom - 100 * 0.5;
  for (let pDot of st.dots) {
    ...
    const fall = (1.2 + 2.2 * Math.abs(Math.sin(Math.PI * (st.t / SETTINGS.Duration) * 0.6))) * 1.1 * pDot.k;
    pDot.x += wind * 3 + sway;
    pDot.y += fall;
    if (pDot.y > capY) {
...
// 2월(이전 2월) kinetic text (SVG y좌표 + 웨이브 모션) - 2월과 동일한 구조, 기호만 콤마
drawFns['2'] = (p, g, st) => {
  if (!st.inited) {
    st.inited = true;
    st.font = loadedFont;
...
// 3월 (March) kinetic text + slash rain - 2월과 동일한 구조, 기호만 슬래시
drawFns['3'] = (p, g, st) => {
  if (!st.inited) {
    st.inited = true;
    st.t = 382.595;
  ...
  for (let idx = 0; idx < copies; idx++) {
    ...
    let xpos = S.SideMargin;
    for (let i = 0; i < S.word.length; i++) {
      g.text(S.word[i], xpos, y);
      xpos += g.textWidth(S.word[i]) + tracking;
    }
  }
  // slash rain (슬래시)
  g.fill(S.fg);
  ...
  g.textSize(38);
  for (let pDot of st.dots) {
    const wind = 0.35 * (p.noise((pDot.y + st.t * 110) * 0.002, (st.t + pDot.theta) * 0.55) - 0.5);
    const drift = 0.6 * Math.sin(st.t * p.TWO_PI * 0.9 + pDot.theta);
    pDot.vx += (wind * 0.08 + drift * 0.02);
    pDot.vy += 0.35 * 0.18 * pDot.k;
    pDot.vy = Math.min(pDot.vy, 5.0 * pDot.k);
    pDot.x += pDot.vx;
    pDot.y += pDot.vy;
    const usableH = g.height - S.TopMargin - S.BottomMargin;
    const copies = p.constrain(Math.floor(usableH / S.Distance), 1, 1200);
