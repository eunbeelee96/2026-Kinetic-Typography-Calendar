...
drawFns['8'] = (p, g, st) => {
  ...
  const SETTINGS = {
    ...
    BottomMargin: 0,
    Distance: 11.5,
    StartTracking: 42,    // 넓은 간격에서 시작
    EndTracking: -12,     // 좁은 간격으로 수렴
    Speed: 0.003,         // 천천히 변화
  ...
  if (!st.inited) {
    ...
    // 세미콜론 파티클 초기화
    st.slashDots = [];
    for (let i = 0; i < SETTINGS.SemicolonCount; i++) {
      const fixedAngle = p.radians(p.random(-80, 80));
      st.slashDots.push({
        x: p.random(0, g.width),
        y: (i / SETTINGS.SemicolonCount) * g.height + p.random(-30, 30),
        baseX: p.random(0, g.width),
        theta: p.random(p.TWO_PI),
        wavePhase: p.random(p.TWO_PI),
        waveSpeed: p.random(0.04, 0.08),
        waveAmplitude: p.random(30, 80),
        k: p.random(0.75, 1.35),
    ...
    // 세미콜론 스프라이트 생성
    const glyphSize = p.max(8, p.floor(SETTINGS.SemicolonSize));
    const padFactor = 2.0;
    const S = p.floor(glyphSize * padFactor);
    st.slashDotSprite = p.createGraphics(S, S);
    st.slashDotSprite.pixelDensity(1);
    st.slashDotSprite.background(0, 0);
    st.slashDotSprite.noStroke();
    st.slashDotSprite.fill(0);
    st.slashDotSprite.textFont(st.font || 'serif');
    st.slashDotSprite.textSize(glyphSize);
    st.slashDotSprite.textAlign(p.CENTER, p.BASELINE);
    const asc = st.slashDotSprite.textAscent();
    const desc = st.slashDotSprite.textDescent();
    const H = asc + desc;
    const padY = (S - H) * 0.5;
    const cx = S * 0.5;
    const baseY = padY + asc;
    st.slashDotSprite.text(SETTINGS.SemicolonGlyph, cx, baseY);
  }
  
  // Background
  g.background(SETTINGS.bg);
  
  // --- August Convergence Animation ---
  g.textFont(st.font || 'IPAMincho Regular');
  g.textSize(SETTINGS.FontSize);
  g.textAlign(p.LEFT, p.CENTER);
  g.noStroke();
  ...
  for (let idx = 0; idx < copies; idx++) {
    ...
    // SVG와 유사한 점진적 수렴 계산
    const timeOffset = rowRatio * 0.3; // 행별 시간 지연
    const adjustedTime = p.max(0, st.t - timeOffset);
    
    // 지수적 수렴 (SVG 패턴과 유사)
    const convergence = 1 - p.exp(-adjustedTime * 1.2);
    
    // 트래킹 계산: 넓은 간격에서 매우 좁은 간격으로
    const tracking = p.lerp(SETTINGS.StartTracking, SETTINGS.EndTracking, convergence);
    
    // 투명도: 중앙으로 갈수록 진하게
    const centerDistance = p.abs(rowRatio - 0.5) * 2;
    const baseAlpha = p.max(0.2, 1 - centerDistance * 0.8);
    const convergenceAlpha = 0.3 + convergence * 0.7;
    const alpha = baseAlpha * convergenceAlpha;
    ...
    const x = SETTINGS.SideMargin;
    drawAugustTrackedText(p, g, SETTINGS.word, x, y, tracking, st.t + idx * 0.02, SETTINGS.FontSize, idx);
  }
  
  // --- Semicolon Rain Overlay ---
  if (SETTINGS.EnableSemicolons) {
    for (let pDot of st.slashDots) {
      // 파도 모션: 부드러운 sin파 곡선으로 좌우 움직임
      pDot.wavePhase += pDot.waveSpeed;
      
      // sin파를 이용한 좌우 움직임
      const waveOffset = p.sin(pDot.wavePhase) * pDot.waveAmplitude;
      const waveRipple = p.sin(pDot.y * 0.01 + pDot.wavePhase) * pDot.waveAmplitude * 0.5;
      pDot.x = pDot.baseX + waveOffset + waveRipple;
      
      // 상하로도 미세한 움직임
      const verticalRipple = p.sin(pDot.x * 0.008 + pDot.wavePhase * 0.7) * 8;
      pDot.y += verticalRipple * 0.02;
      
      // 경계 처리
      if (pDot.y < -50) pDot.y = g.height + 50;
      if (pDot.y > g.height + 50) pDot.y = -50;
      
      // 기준 X 위치도 천천히 변화
      pDot.baseX += p.sin(st.t * 0.012 + pDot.theta) * 0.5;
      if (pDot.baseX < -100) pDot.baseX = g.width + 100;
      if (pDot.baseX > g.width + 100) pDot.baseX = -100;
      
      // 고정된 각도로 회전
      pDot._angle = pDot.fixedAngle;
      
      g.push();
      g.translate(p.round(pDot.x), p.round(pDot.y));
      if (pDot._angle !== 0) g.rotate(pDot._angle);
      g.image(st.slashDotSprite, 0, 0);
      g.pop();
    }
  }
...
// Helper function for August tracked text rendering
function drawAugustTrackedText(p, g, str, x, y, tracking, tt, baseFontSize, rowIndex) {
  let cx = x;
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    // 아주 미세한 글자별 움직임
    const letterTime = tt + i * 0.05;
    const yOffset = p.sin(letterTime * p.PI * 4) * 0.3;
    const xOffset = p.cos(letterTime * p.PI * 3) * 0.2;
    
    g.push();
    g.translate(cx + xOffset, y + yOffset);
    g.text(ch, 0, 0);
    g.pop();
...
drawFns['1'] = (p, g, st) => {
  ...
  if (!st.inited) {
    ...
    st.svgTextData = [
      ...
      [144.00, 682.00, 'f'], [149.00, 682.00, 'e'], [154.00, 682.00, 'b'], [159.00, 682.00, 'r'], [164.00, 682.00, 'u'], [169.00, 682.00, 'a'], [174.00, 682.00, 'r'], [179.00, 682.00, 'y'],
      [146.50, 697.50, 'f'], [151.00, 697.50, 'e'], [155.50, 697.50, 'b'], [160.00, 697.50, 'r'], [164.50, 697.50, 'u'], [169.00, 697.50, 'a'], [173.50, 697.50, 'r'], [178.00, 697.50, 'y'],
      [143.00, 713.00, 'f'], [147.50, 713.00, 'e'], [152.00, 713.00, 'b'], [156.50, 713.00, 'r'], [161.00, 713.00, 'u'], [165.50, 713.00, 'a'], [170.00, 713.00, 'r'], [174.50, 713.00, 'y'],
    ];
    // 콤마 rain 초기화
    for (let i = 0; i < 14; i++) {
      st.dots.push({
