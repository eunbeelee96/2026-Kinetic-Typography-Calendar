...
drawFns['8'] = (p, g, st) => {
  ...
  if (!st.inited) {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      st.slashDots.push({
        x: p.random(0, g.width),
        y: (i / 14) * g.height + p.random(-30, 30),
    ...
    // 세미콜론 스프라이트 생성
    const glyphSize = p.max(8, p.floor(40));
    const padFactor = 2.0;
    ...
    const cx = S * 0.5;
    const baseY = padY + asc;
    st.slashDotSprite.text(';', cx, baseY);
  }
  ...
  for (let pDot of st.slashDots) {
    // 파도 모션: 각 파티클이 부드러운 sin파 곡선으로 좌우 움직임
    pDot.wavePhase += pDot.waveSpeed;
    
    // sin파를 이용한 부드러운 좌우 움직임 - 물결 모양으로 연결된 파도
    const waveOffset = p.sin(pDot.wavePhase) * pDot.waveAmplitude;
    // Y 위치에 따라 파도의 위상이 다르게 적용되어 물결 모양 생성
